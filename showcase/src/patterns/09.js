...
const useClapState = (initialState = INITIAL_STATE) => {
  ...
  const [clapState, setClap] = useState(initialState);

  const reset = useCallback(() => {
    setClap(initialState);
  }, []);

  const setClapState = useCallback(() => {
    setClap(({ count, countTotal }) => ({
      isClicked: true,
  ...
  const getTogglerProps = ({ onClick, ...otherProps }) => ({
    onClick: callFnsInSequence(setClapState, onClick),
  });
  ...
  return { clapState, setClapState, getTogglerProps, getCounterProps, reset };
};
